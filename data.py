...
def load_data(data_dir, task, k, seed, split):
    if os.path.exists('{}/{}/{}-{}'.format(data_dir,task,k,seed)):
        data_dir = '{}/{}/{}-{}'.format(data_dir,task,k,seed)
    else:
...
def prepare_data(tokenizer, train_data, test_data, max_length, max_length_per_example,
                 ...
                 ensemble=False, is_null=False,shuffle_seed=-1,warning_truncated=True):
    ...
    if use_demonstrations:
        ...
        if shuffle_seed>0:
            random.Random(shuffle_seed).shuffle(train_data)
            logger.info('train_data after shuffle')
            for i, example in enumerate(train_data):
                logger.info('example {}: {}'.format(i,example))
